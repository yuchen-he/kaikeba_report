...
class SHOW_IMG:                         # 进行图片初始显示及拼接后显示
    ...
    def bgr2rgb(self, img):             # 通道转换BGR2RGB
        return cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
    def rgb2bgr(self, img):             # 通道转换BGR2RGB
    ...
    def show_2_img(self, a, b):         # 显示两张图片
        ...
        plt.show()
    def get_perspective_point(self, T, point):
        src_p = np.float32([[[point[0], point[1]]]])
        dst_p = cv2.perspectiveTransform(src_p, T)
        return (dst_p[0][0][0], dst_p[0][0][1])
    def add_bounding_box(self, a_shape, b, T):
        a_width, a_high, a_chl = a_shape
        corner_0, corner_1, corner_2, corner_3 = [0, 0], [0, a_width], [a_high, 0], [a_high, a_width]
        corner_0_p, corner_1_p, corner_2_p, corner_3_p = self.get_perspective_point(T, corner_0), \
                                                         self.get_perspective_point(T, corner_1), \
                                                         self.get_perspective_point(T, corner_2), \
                                                         self.get_perspective_point(T, corner_3)
        background = b.copy()
...
class SIFT:                             # SIFT计算
    ...
    def get_sift_image(self, img, kp):      # 生成SIFT关键点显示图
...
class RANSAC:
    ...
    def get_good_nearest_sift_kp(self, single_kp1_index):               # 获得高质量的尺度不变关键点
        ...
        min, min_2nd = diff_sort[0], diff_sort[1]
        if min/min_2nd < self.good_kp_threshold:
            good_label = 1
    ...
    def get_correspond_4point(self, conrespond_table):                      # 获取与随机点对应的4个点
        src_4p, dst_4p = [], []
        while len(src_4p) < 4:
    ...
    def count_inner_point(self, conrespond_table):                                   # 计算内点数量
        T = self.get_homography(conrespond_table)
        # 右乘投影矩阵得到初步的投影点
        ori_perspective = np.dot(np.column_stack((conrespond_table[:, 1:3], np.ones(len(conrespond_table)))), T.T)
...
class OBJECT_TRANCK():                                                              # 物体追踪
    ...
    def draw_target_box_by_move_mouse(self, event, x, y, flags, param):             # 选择追踪目标
        global ini_frame, point1, point2
    ...
    def tracking_one_frame(self, img_target_shape, img_frame, max_iteration, inner_ratio,   # 在单一帧中匹配
                           threshold, kp_target, des_target, kp_frame, des_frame):
        inner_count, T = RANSAC(max_iteration, inner_ratio, threshold, kp_target, des_target
                                , kp_frame, des_frame).compare_refresh()
        return show_img.add_bounding_box(img_target_shape, img_frame, np.array(T))
    def tracking(self):                                                                     # 所有帧中匹配
        ...
        while available:
            ...
            if i % int(10) == 0:
                ...
                plt.imshow(show_img.bgr2rgb(frame_boxed))
                plt.show()
    ...
    def play_video(self):                                                                       # 播放匹配结果
        ...
        cv2.putText(frame, 'Push ESC to Quit', (int(self.fsize[0] / 2), int(self.fsize[1] / 2)), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (55, 255, 155), 2)
        while available:
...
if __name__ == '__main__':
    show_img = SHOW_IMG()                                       # 显示图片
    object_tracking = OBJECT_TRANCK('./object_tracking_video.mp4', 2000, 0.5, 1)
    frames = object_tracking.tracking()
