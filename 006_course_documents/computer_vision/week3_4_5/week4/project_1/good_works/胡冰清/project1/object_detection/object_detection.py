...
class SHOW_IMG:                         # 图片显示
    ...
    def show_2_img(self, a, b):         # 显示两张图片
        ...
        plt.show()
    def get_perspective_and_translation_point(self, T, point, translation_length):
    ...
    def show_bounding_box_and_mapping_line(self, a, b, T, conrespond_table, inner_point_index):
        ...
        corner_0_p, corner_1_p, corner_2_p, corner_3_p = self.get_perspective_and_translation_point(T, corner_0, a_high), \
                                                         self.get_perspective_and_translation_point(T, corner_1, a_high), \
                                                         self.get_perspective_and_translation_point(T, corner_2, a_high), \
                                                         self.get_perspective_and_translation_point(T, corner_3, a_high)
        #  绘制投影框
...
class RANSAC:                               # RANSAC计算类
    ...
    def get_good_nearest_sift_kp(self, single_kp1_index, column_sorted, sorted_des2):     # 获得高质量的尺度不变关键点
        ...
        if len(diff_neighbor) > 1:
            ...
            min, min_2nd = diff_neighbor_sort[0], diff_neighbor_sort[1]
    ...
    def count_inner_point(self, conrespond_table):                                  # 计算内点数量
        ...
        return np.sum(result), T, np.argwhere(result == 1).flatten()
    def compare_refresh(self):                                                      # 比较内点数量，刷新内点数量和单应性矩阵
        ...
        for _i in range(self.max_iter):
            ...
            if new_inner_count > inner_count:
                ...
                show_img.show_bounding_box_and_mapping_line(img1, img2, np.array(T), conrespond_table, inner_point_index)
            print("iteration:{}  inner_count:{}".format(_i, inner_count))
            # print("iteration:{}  T:{}".format(_i, T))
        ...
        print("T:\n", T)
        show_img.save_img(show_img.show_bounding_box_and_mapping_line(img1, img2, np.array(T), conrespond_table, inner_point_index))
...
if __name__ == '__main__':
    ...
    show_img.show_2_img(img1, img2)                             # 显示源图
