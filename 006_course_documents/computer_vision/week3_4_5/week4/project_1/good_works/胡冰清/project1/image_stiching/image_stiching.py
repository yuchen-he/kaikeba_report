...
class SHOW_IMG:                         # 进行图片初始显示及拼接后显示
    ...
    def show_combine_img(self, a, b, T):
        ...
        cut_img = self.cut_img(background, a, b)
        plt.imshow(cut_img)
        plt.show()
        return cut_img
    def cut_img(self, img, a, b):
...
class RANSAC:
    def __init__(self, max_iter, inner_ratio, threshold, kp1, des1, kp2, des2):    # 初始化
        ...
        self.neighbor_threshold = 5         # 近邻排序相对于输入点的上下限偏移
    ...
    def compare_refresh(self):                                          # 比较内点数量，刷新内点数量和单应性矩阵
        ...
        for _i in range(self.max_iter):
            # print("iteration",_i)
            new_inner_count, new_T = self.count_inner_point(conrespond_table)
            if new_inner_count > inner_count:
                inner_count = new_inner_count
                T = new_T
                show_img.show_combine_img(img1, img2, np.array(T))
        ...
        print("T:\n", T)
        show_img.save_img(show_img.show_combine_img(img1, img2, np.array(T)))
